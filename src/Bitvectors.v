...
Section Bits.
  ...
  Record Int {n : nat} : Type := mkint { intval: Z; intrange: -1 < intval < two_power_nat n }.


  Definition modulus {ws : nat} : Z := two_power_nat ws.
  Definition zwordsize {ws : nat} : Z := Z_of_nat ws.
  Definition half_modulus {ws : nat} : Z := (@modulus ws) / 2.
  Definition max_unsigned {ws : nat} : Z := (@modulus ws) - 1.
  Definition max_signed {ws : nat} : Z := (@half_modulus ws) - 1.
  Definition min_signed {ws : nat} : Z := - (@half_modulus ws).

  (** The [unsigned] and [signed] functions return the Coq integer corresponding
  to the given machine integer, interpreted as unsigned or signed
  respectively. *)

  Definition unsigned {ws : nat} (n: @Int ws) : Z := intval n.

  Definition signed {ws : nat} (n: @Int ws) : Z :=
    let x := unsigned n in
    if zlt x ((two_power_nat ws) / 2) then x else x - (two_power_nat ws).
  ...
  Definition Z_mod_modulus {ws : nat} (x: Z) : Z :=
    match x with
    | Z0 => 0
    | Zpos p => P_mod_two_p p ws
    | Zneg p => let r := P_mod_two_p p ws in if zeq r 0 then 0 else (@modulus ws) - r
    end.


  Remark modulus_power {ws : nat} : (@modulus ws) = two_p (@zwordsize ws).
  Proof.
  ...
  Definition lt {ws : nat} (x y: @Int ws) : bool :=
...
>>
  ...
  Definition cmp {ws : nat} (c: comparison) (x y: @Int ws) : bool :=
    match c with
    | Ceq => eq x y
    | Cne => negb (eq x y)
    | Clt => lt x y
  ...
  Definition cmpu {ws : nat} (c: comparison) (x y: @Int ws) : bool :=
    match c with
    | Ceq => eq x y
    | Cne => negb (eq x y)
    | Clt => ltu x y
  ...
  Lemma mkint_eq:
    forall {ws} x y Px Py, x = y -> @mkint ws x Px = @mkint ws y Py.
  Proof.
    intros. subst y.
  ...
  Lemma eq_dec: forall {ws : nat} (x y: @Int ws), {x = y} + {x <> y}.
  Proof.
...
Proof.
  intros.
  pose (bits_size_2 a) as Hbsa.
  pose (bits_size_2 b) as Hbsb.
  
  assert (0 <= Z.min (size a) (size b)).
  {
    generalize (size_range a) (size_range b). intros.
    rewrite Zmin_spec. destruct (zlt (size a) (size b)); omega.
  } idtac.

  
  apply bits_size_3. auto. intros.

  remember H0 as Hrange; clear HeqHrange.
  rewrite Zmin_spec in H0.

  destruct (zlt (size a) (size b));
    rewrite bits_and by omega.
  
  * rewrite Hbsa; auto; omega.
  * rewrite Hbsb. apply andb_false_r. omega.
Qed.
