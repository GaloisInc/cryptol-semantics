...
testIdentity = plain_block == plain_block'
  where {
    key = "This is a key.  ";
    sched = keySchedule key;
    plain_block = "This is a test. ";
    cipher_block = encrypt (plain_block, sched);
    plain_block' = decrypt (cipher_block, sched);
  };

// Expected ciphertext for given key on all zero plaintext
knownVectors
  = [ (0x80000000000000000000000000000000{8*4*Nk},
       0x0EDD33D3C621E546455BD8BA1418BEC8{8*4*Nk}),
      (0x40000000000000000000000000000000,
       0xC0CC0C5DA5BD63ACD44A80774FAD5222),
      (0x00000000000000020000000000000000,
       0x5C76002BC7206560EFE550C80B8F12CC),
      (0x00000000000000000002000000000000,
       0xF6BBA5C10DB02529E2C2DA3FB582CC14),
      (0x00000000000000000000000000000001,
       0x0545AAD56DA2A97C3663D1432A3D1C84) ];

testAgainstKnown =
  [ join (aes (zero, split key)) == ciphertext /\
    sea (split ciphertext, split key) == zero
      | (key, ciphertext) <- knownVectors ] == limit; 

testAll = 
  if mcc_target == "srce-shal" /\ mcc_trans != "cano" then
    /* Too much recursion for source-shallow with recursive streams */
    (| true |)
  else if mcc_target == "aamp-deep" then
    /* AAMP model runs out of memory (runs ok on simulator though) */
    (| true |)
  else
    (| testIdentity /\ testAgainstKnown |);
